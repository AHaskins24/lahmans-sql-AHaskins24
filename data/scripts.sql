...
2. Find the name and height of the shortest player in the database. How many games did he play in? What is the name of the team for which he played?
   SELECT ppl.namefirst, ppl.namelast, t.name, g_all AS games_played, MIN(height) AS height 
   FROM people AS ppl
   LEFT JOIN appearances AS app
   ON ppl.playerid = app.playerid
   LEFT JOIN teams AS t
   ON app.teamid = t.teamid
   GROUP BY ppl.namefirst, ppl.namelast, t.name, g_all
...
SELECT namesfirst, nameslast, schoolname
FROM 
    (SELECT )
